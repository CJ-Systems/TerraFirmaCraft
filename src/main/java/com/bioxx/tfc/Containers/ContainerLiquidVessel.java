    ...
    public void detectAndSendChanges() {
        ...
        if (nbt != null) {
            ...
            if (input != null && input.getItem() instanceof ItemPotteryMold
                && input.getMetadata() > 1 /* Ceramic */
                && input.getMetadata() <= 5 /* Full of Metal */)
            ...
            if (!world.isRemote && m != null && stack != null) {
                if (input != null && input.getItem() == TFCItems.ceramicMold
                ...
                } else if (input != null && input.getItem() == m.meltedItem && input.getMetadata() > 0) {
                    input.setMetadata(input.getMetadata() - 1);
                    TFC_ItemHeat.setTemp(
                        input,
                        (short) (HeatRegistry.getInstance()
                    ...
                    if (input != null && input.getItem() instanceof ItemPotteryMold && input.getMetadata() > 1) {
                        boolean correctMetalFlag = false;
                        if ("Copper".equals(m.name) && (input.getMetadata() - 2) % 4 == 0) correctMetalFlag = true;
                        else if ("Bronze".equals(m.name) && (input.getMetadata() - 2) % 4 == 1) correctMetalFlag = true;
                        else if ("Bismuth Bronze".equals(m.name) && (input.getMetadata() - 2) % 4 == 2)
                            correctMetalFlag = true;
                        else if ("Black Bronze".equals(m.name) && (input.getMetadata() - 2) % 4 == 3)
                            correctMetalFlag = true;

                        if (correctMetalFlag) {
                            if (input.getMetadata() > 5) {
                                input.setMetadata(input.getMetadata() - 4);
                                TFC_ItemHeat.setTemp(
                                    input,
                                    (short) (HeatRegistry.getInstance()
                                        .getMeltingPoint(input) * 1.5f));
    ...
    public ItemStack transferStackInSlotTFC(EntityPlayer player, int slotNum) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotNum < 1) {
                ...
                && (slotStack.getItem() == TFCItems.ceramicMold && slotStack.getMetadata() == 1
                    || slotStack.getItem() instanceof ItemMeltedMetal && slotStack.getMetadata() > 1
                    || slotStack.getItem() instanceof ItemPotteryMold && slotStack.getMetadata() > 0)) {
