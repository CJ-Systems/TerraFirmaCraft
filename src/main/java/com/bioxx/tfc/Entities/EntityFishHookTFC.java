...
public class EntityFishHookTFC extends EntityFishHook {
    ...
    public EntityFishHookTFC(World par1World, EntityPlayer par2EntityPlayer, int ticks) {
        ...
        this.motionZ = MathHelper.cos(this.rotationYaw / 180.0F * (float) Math.PI)
            ...
            * f;
        this.motionY = -MathHelper.sin(this.rotationPitch / 180.0F * (float) Math.PI) * f;
        this.handleHookCasting(this.motionX, this.motionY, this.motionZ, tickRatio/* 1.5F */, 1.0F);
    }
    ...
    @Override
    public void handleHookCasting/* calculateVelocity */(double par1, double par3, double par5, float par7,
        float par8) {
    ...
    public void onUpdate() {
        ...
        if (this.getDistanceToEntity(this.angler) < 1) {
            this.setDead();
            if (this.angler.getHeldItem() != null) {
        ...
        if (this.fishPosRotationIncrements > 0) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                if (this.angler.isDead || !this.angler.isEntityAlive()
                    ...
                    || this.getDistanceSqToEntity(this.angler) > 2500.0D) {
                    this.setDead();
            ...
            if (!this.inGround) {
                ...
                if (d6 > 0.0D) {
                    if (this.maxDistance == -1) {
                        this.maxDistance = this.getDistanceToEntity(angler);
                ...
                double distance = this.getDistanceToEntity(this.angler);
                if (distance > this.maxDistance && maxDistance != -1) {
                    Vec3 distVec = Vec3.createVectorHelper(
                        this.posX - this.angler.posX,
                        this.posY - this.angler.posY,
                        this.posZ - this.angler.posZ);
                    double distRatio = maxDistance / distance;
                    this.posX = this.angler.posX + (distVec.xCoord * distRatio);
                    this.posY = this.angler.posY + (distVec.yCoord * distRatio);
                    this.posZ = this.angler.posZ + (distVec.zCoord * distRatio);
                }
                this.setPosition(this.posX, this.posY, this.posZ);
            }
        }
         ...
         * Vec3 distVec = Vec3.createVectorHelper(this.posX - this.angler.posX,this.posY - this.angler.posY,this.posZ -
         * this.angler.posZ);
         * double distRatio = maxDistance / distance;
         * this.posX = this.angler.posX + (distVec.xCoord * distRatio);
         * this.posY = this.angler.posY + (distVec.yCoord * distRatio);
    ...
    public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound) {
        ...
        par1NBTTagCompound.setByte("shake", (byte) this.shake/* shake */);
    ...
    public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound) {
        ...
        this.shake/* shake */ = par1NBTTagCompound.getByte("shake") & 255;
    ...
    public Vec3 applyEntityForce(Vec3 entityForce, double x, double y, double z) {
        ...
        ItemStack is = this.angler.getHeldItem();
        ...
        if (lineTension > MAX_LINE_TENSION * 0.8 && !lineTensionSnap) {
            ...
            TFC_Core.sendInfoMessage(this.angler, new ChatComponentTranslation("fishingRod.lineTension"));
        } else if (lineTension < MAX_LINE_TENSION * 0.8) {
            lineTensionSnap = false;
        }
        if (lineTension >= MAX_LINE_TENSION) {
            this.angler.getCurrentEquippedItem()
                .damageItem(20, angler);
            this.ridingEntity.riddenByEntity = null;
            this.ridingEntity = null;
            // ((EntityLiving)this.ridingEntity).dismountEntity(this);
            TFC_Core.sendInfoMessage(this.angler, new ChatComponentTranslation("fishingRod.lineSnap"));
            this.setDead();
        }
        this.pullX = 0;
        this.pullY = 0;
        ...
        // this.angler.addChatMessage(new ChatComponentText("tension: " + lineTension + ", force ratio: " + forceRatio
        // +", distance: " + this.getDistanceToEntity(angler) + ", max: "+this.maxDistance));
    ...
    public Vec3 getNormalDirectionOfPlayer(double x, double y, double z) {
        Vec3 dirVec = Vec3.createVectorHelper(this.angler.posX - x, this.angler.posY - y, this.angler.posZ - z);
        return dirVec.normalize();
    }

    public Vec3 getTooFarAdjustedVec(Vec3 motionVec, double x, double y, double z) {
        Vec3 playerMotion = Vec3.createVectorHelper(this.angler.motionX, this.angler.motionY, this.angler.motionZ);
        double subractedRatio = Math.max(
            1 - (this.maxDistance
                / this.angler.getDistance(x + playerMotion.xCoord, y + playerMotion.yCoord, z + playerMotion.zCoord)),
            0);
        return Vec3.createVectorHelper(
            (this.angler.posX + playerMotion.xCoord - (motionVec.xCoord + x)) * subractedRatio,
            (this.angler.posY + playerMotion.yCoord - (motionVec.yCoord + y)) * subractedRatio,
            (this.angler.posZ + playerMotion.zCoord - (motionVec.zCoord + z)) * subractedRatio);
    }
    ...
    public boolean isTooFarFromPlayer(double x, double y, double z) {
        return this.angler.getDistance(x, y, z) > this.maxDistance;
    }
    ...
    public int getAverageFishPopFromChunks() {
        if (this.worldObj.isRemote) {
            ...
            EntityPlayer player = this.angler;
    ...
    @Override
    public int handleHookRetraction/* catchFish */() {
        if (this.worldObj.isRemote) {
            return 0;
        } else {
            EntityPlayer player = this.angler;
    ...
    public void setDeadKill() {
        if (this.ridingEntity != null && this.ridingEntity instanceof EntityLiving) {
            ...
            ((EntityLiving) (this.ridingEntity)).attackEntityFrom(new EntityDamageSource("fishing", angler), 1);
