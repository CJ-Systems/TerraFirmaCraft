...
public class TEAnvil extends NetworkTileEntity implements IInventory {
    ...
    public void updateRecipe() {
        ...
        if (anvilItemStacks[INPUT1_SLOT] != null && anvilItemStacks[INPUT1_SLOT].getItem() == TFCItems.bloom) {
            if (anvilItemStacks[INPUT1_SLOT].getMetadata() <= 100 && planList.containsKey("splitbloom"))
        ...
        if (anvilItemStacks[INPUT1_SLOT] != null && anvilItemStacks[INPUT1_SLOT].getItem() == TFCItems.bloom
            ...
            if (anvilItemStacks[INPUT1_SLOT].getMetadata() < 100) {
                craftingPlan = "";
                workRecipe = null;
            } else if (anvilItemStacks[INPUT1_SLOT].getMetadata() == 100) {
    ...
    private void damageHammer() {
        anvilItemStacks[HAMMER_SLOT].setMetadata(anvilItemStacks[HAMMER_SLOT].getMetadata() + 1);
        if (anvilItemStacks[HAMMER_SLOT].getMetadata() == anvilItemStacks[HAMMER_SLOT].getMaxDurability())
    ...
    private boolean canBeWorked() {
        return isTemperatureWorkable(INPUT1_SLOT) && anvilItemStacks[HAMMER_SLOT] != null
            && (anvilItemStacks[INPUT1_SLOT].getMetadata() == 0 || anvilItemStacks[INPUT1_SLOT].getItem()
                .getHasSubtypes())
    ...
    public void actionWeld() {
        if (!worldObj.isRemote) {
            if (isTemperatureWeldable(WELD1_SLOT) && isTemperatureWeldable(WELD2_SLOT)
                && anvilItemStacks[HAMMER_SLOT] != null
                && (anvilItemStacks[WELD1_SLOT].getMetadata() == 0 || anvilItemStacks[WELD1_SLOT].getItem()
                    .getHasSubtypes())
                && (anvilItemStacks[WELD2_SLOT].getMetadata() == 0 || anvilItemStacks[WELD2_SLOT].getItem()
                    .getHasSubtypes())
                && workedRecently == 0
        ...
        if (TFC_ItemHeat.hasTemp(is)) {
            ...
            if (index != null) {
                ...
                if (temp < index.meltTemp && temp > weldTemp) {
                    ...
                    return !(is.getItem() instanceof ItemMeltedMetal) || is.getMetadata() == 0;
                }
            }
        }
        return false;
