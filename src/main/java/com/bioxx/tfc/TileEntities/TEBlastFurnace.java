...
public class TEBlastFurnace extends TEFireEntity implements IInventory {
    ...
    public void cookItem(int i) {
        ...
        if (cookingItemStack != null && crucibleTE != null
            ...
            if (index != null && TFC_ItemHeat.getTemp(cookingItemStack) >= index.meltTemp) {
                ...
                if (storage[1] != null && storage[1].getMetadata() == storage[1].getMaxDurability()) {
        ...
        if (!worldObj.isRemote) {
            ...
            if (list != null && !list.isEmpty()
                ...
                for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                    ...
                    if (item == TFCItems.coal && itemstack.getMetadata() == 1 /*
                                                                               * ||
                                                                               * item == TFCItems.Coke
                                                                               */) {
                            ...
                            for (; c > 0; c--) {
                                if (getTotalCount() < 40 && oreCount < (this.maxValidStackSize * 4)) {
                                    if (foundFlux(moltenCount)
                                        && addOreToFire(new ItemStack(item, 1, itemstack.getMetadata()))) oreCount += 1;
                                    else nonConsumedOre++;
                                } else {
                                    nonConsumedOre++;
                                }
    ...
    private boolean foundFlux(int moltenCount) {
        ...
        for (Iterator iterator = list.iterator(); iterator.hasNext() && !found;) {
            ...
            if (!entity.isDead && is.getMetadata() == 0 && is.getItem() == TFCItems.powder) {
                is.stackSize--;
