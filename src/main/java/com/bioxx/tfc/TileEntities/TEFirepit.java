...
public class TEFirepit extends TEFireEntity implements IInventory {
    ...
    public void combineMetals(ItemStack inputItem, ItemStack destItem) {
        int d1 = 100 - inputItem.getMetadata();
        int d2 = 100 - destItem.getMetadata();
        // This was causing the infinite amounts possibly
        destItem.setMetadata(100 - (d1 + d2));
    }
    ...
    public void cookItem() {
        ...
        if (fireItemStacks[1] != null) {
            ...
            if (index != null && TFC_ItemHeat.getTemp(fireItemStacks[1]) > index.meltTemp) {
                ...
                ItemStack mold = null;
                if (output != null) {
                    damage = output.getMetadata();
                    if (output.getItem() == fireItemStacks[1].getItem()) damage = fireItemStacks[1].getMetadata();
                    ...
                    if (fireItemStacks[1].getItem() instanceof ItemMeltedMetal) {
                        ...
                        // in the second output slot
                            ...
                            && (fireItemStacks[7].getItem() != fireItemStacks[1].getItem()
                                || fireItemStacks[7].getMetadata() == 0)) {
                ...
                if (output != null && output.getItem() instanceof ItemMeltedMetal) {
                    ...
                    if (fireItemStacks[7] != null && output.getItem() == fireItemStacks[7].getItem()
                        ...
                        int amt2 = 100 - fireItemStacks[7].getMetadata();// the percentage currently in the out slot
                        ...
                        fireItemStacks[7].setMetadata(amt4);
                        ...
                        && fireItemStacks[8].getMetadata() > 0) {
                            int amt1 = 100 - damage;// the percentage of the output
                            int amt2 = 100 - fireItemStacks[8].getMetadata();// the percentage currently in the out slot
                            int amt3 = amt1 + amt2;// combined amount
                            ...
                            fireItemStacks[8].setMetadata(amt4);
                    ...
                    if (addLeftover) {
                        ...
                        if (fireItemStacks[dest] != null && output.getItem() == fireItemStacks[dest].getItem()
                            && fireItemStacks[dest].getMetadata() > 0) {
                            int amt1 = 100 - leftover;// the percentage of the output
                            int amt2 = 100 - fireItemStacks[dest].getMetadata();// the percentage currently in the out
                                                                                // slot
                            int amt3 = amt1 + amt2;// combined amount
                            int amt4 = 100 - amt3;// convert the percent back to mc damage
                            if (amt4 < 0) amt4 = 0;// stop the infinite glitch
                            fireItemStacks[dest] = output.copy();
                            fireItemStacks[dest].setMetadata(amt4);
                            ...
                            && fireItemStacks[dest].getItem() == TFCItems.ceramicMold) {
                                fireItemStacks[dest] = output.copy();
                                fireItemStacks[dest].setMetadata(100 - leftover);
                                TFC_ItemHeat.setTemp(fireItemStacks[dest], temp);
                            }
                    }
                } else if (output != null) {
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
                    ...
                    if (item == TFCItems.logs || item == Item.getItemFromBlock(TFCBlocks.peat)) {
                        for (int c = 0; c < is.stackSize; c++) {
                            if (fireItemStacks[0] == null) // Secondary check for empty input slot.
                            {
                                 ...
                                 */
                                setInventorySlotContents(0, new ItemStack(item, 1, is.getMetadata()));
                                is.stackSize--;
             ...
             * fireItemStacks[1].getMetadata() == 1;
             */
