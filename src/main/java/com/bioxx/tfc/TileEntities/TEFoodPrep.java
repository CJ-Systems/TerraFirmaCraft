...
public class TEFoodPrep extends NetworkTileEntity implements IInventory {
    ...
    private void setSandwichIcon(ItemStack is) {
        if (getStackInSlot(0).getItem() == TFCItems.wheatBread) is.setMetadata(0);
        else if (getStackInSlot(0).getItem() == TFCItems.oatBread) is.setMetadata(1);
        else if (getStackInSlot(0).getItem() == TFCItems.barleyBread) is.setMetadata(2);
        else if (getStackInSlot(0).getItem() == TFCItems.ryeBread) is.setMetadata(3);
        else if (getStackInSlot(0).getItem() == TFCItems.cornBread) is.setMetadata(4);
        else if (getStackInSlot(0).getItem() == TFCItems.riceBread) is.setMetadata(5);
    }
    ...
    private void createSalad(EntityPlayer player) {
        if (validateSalad())// Bread
        {
            ...
            is.setMetadata(new Random(getIconSeed()).nextInt(((ItemTerra) TFCItems.salad).metaIcons.length));
    ...
    public boolean validateSalad() {
        if (lastTab == 1) {
            ...
            if (bowlStack == null || bowlStack.getMetadata() != 1) {
                return false;
            }
        }
        return true;
