...
public class ShapedRecipesTFC implements IRecipe {
        ...
        for (int k = 0; k < 5; k++) {
            for (int l = 0; l < 5; l++) {
                ...
                } else if (inputIS == null || recipeIS == null) // No need for XOR since the X is handled above
                {
                    ...
                    if (recipeIS.getMetadata() != 32767 && recipeIS.getMetadata() != inputIS.getMetadata()) {
                        return false;
                    }
                }
    ...
    public ItemStack getCraftingResult(InventoryCrafting inventorycrafting) {
        return new ItemStack(recipeOutput.getItem(), recipeOutput.stackSize, recipeOutput.getMetadata());
    }
